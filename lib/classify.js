...
// og:type 값 → 대분류. "product.item", "video.other", "video.movie" 같은 변형까지 포함 매칭.
const OG_TYPE_MAP = [
  ["product", "shopping"],
  ["video", "video"],
  ["article", "news"],
...
// 룰 기반 분류 (동기, 비용 0): 1. 도메인 룰 → 2. og:type 메타 → 3. 폴백(기타).
...
// 전체 파이프라인: 룰 기반으로 확정되면 거기서 끝, 못 정한 것만 LLM 폴백.
// LLM은 GEMINI_API_KEY가 설정된 경우에만 동작(없으면 룰 기반 결과 그대로).
// categories: [{ slug, name, subs: [하위 이름] }] — LLM 프롬프트에 기존 카테고리로 제공
export async function classifyLink(meta, categories = []) {
  const ruled = classify({ domain: meta.domain, ogType: meta.ogType });
  if (ruled.method !== "fallback") return ruled;
